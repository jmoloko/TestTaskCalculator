...
class RomNumberEvaluate implements ExpressionEvaluate {
    ...
    public Expression expEval(String input) {
        ...
        while (position > 0) {
            ...
            if (Utils.isPresentInRom(Character.toString(inputWS.charAt(position)))) {
            ...
            } else if (Character.isDigit(inputWS.charAt(position))) {
                ...
                Expression right = new NumberExpression(Utils.romToArab(inputWS.substring(position + 1)));
                Operator operator = Operator.fromChar(inputWS.charAt(position));
                return new AcceptDiffExpressions(left, right, operator);
            }
        }
        int result = Utils.romToArab(inputWS);
